...
function convertImage(
    ...
    const command = `magick "${inputPath}" ${options.join(' ')} -resize ${dimensions} "${outputPath}"`;
    try {
...
export async function generateMetaImages() {
    ...
    if (!existsSync(inputImage)) {
        ...
        console.error('Please make sure metaImage.jpg exists in the scripts directory');
        process.exit(1);
    }
    ...
    try {
        ...
        convertImage(inputImage, join(outputDir, 'android-chrome-192x192.png'), '192x192');
        convertImage(inputImage, join(outputDir, 'android-chrome-512x512.png'), '512x512');
        convertImage(inputImage, join(outputDir, 'apple-touch-icon.png'), '180x180');
